...
When invoked, always do all of the following:

1. Analyze the requested scope (single file, module, or full change set).
2. Produce findings with severity, evidence, impact, and concrete fix guidance.
3. Create or update a review document in docs/code-review.
4. Include a clear go/no-go production readiness recommendation.

If no vulnerabilities are found, explicitly document that result and list residual risks or testing gaps.
...
```
    ...
    - Category: OWASP/LLM/Zero-Trust/Reliability mapping
    - Evidence: file path, function/class, and vulnerable code snippet
    - Impact: practical exploit or business risk
    - Fix: minimal safe code change and validation approach
