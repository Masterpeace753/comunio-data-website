...
1. Snapshot file handling from environment path should be hardened
1. Security gates must be release-blocking
...
1. High: security logging should be structured/auditable
1. Medium: database URL policy validation should be stronger (including SSL policy checks)
1. Medium/Low: reflection-based object extraction should move to explicit allowlist mapping
...
1. DB TLS enforcement in connection helper (`sslmode=require` or stronger)
1. Sanitized structured ingest logging with `event/stage/error_code`
1. Snapshot file hardening (base directory allowlist, max size, schema checks)
1. Automated validation tests in `backend/tests/test_security_policies.py`
